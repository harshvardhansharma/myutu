// BISECT: face_check plus I2C traffic, nothing else changed.
...
// Same drawing as face_check. The ONLY addition is I2C polling at the pet's rate.
// If the image shifts here but not in face_check, I2C is corrupting the display —
// which means a display control line shares a breadboard row with an I2C wire.
void setup() {
  Serial.begin(115200);
  delay(300);
  ...
  Wire.write(0x6B); Wire.write(0x00);       // wake the MPU
...
void loop() {
  for (int e = 0; e < 6; e++) {
    ...
    draw(e);
    // Poll I2C at the pet's rate for 3 s: 375 reads, same as the real sketch.
    for (int i = 0; i < 375; i++) {
      uint8_t b[14];
      Wire.beginTransmission(0x68);
      Wire.write(0x3B);
      if (Wire.endTransmission(false) == 0 && Wire.requestFrom(0x68, 14) == 14) {
        for (int k = 0; k < 14; k++) b[k] = Wire.read();
      }
      delay(8);
    }
  }
